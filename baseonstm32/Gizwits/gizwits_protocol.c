...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagset_heartH)
    {
        info->event[info->num] = EVENT_set_heartH;
        info->num++;
        dataPoints->valueset_heartH = gizX2Y(set_heartH_RATIO,  set_heartH_ADDITION, issuedData->attrVals.valueset_heartH); 
    }
        
    if(0x01 == issuedData->attrFlags.flagset_heartL)
    {
        info->event[info->num] = EVENT_set_heartL;
        info->num++;
        dataPoints->valueset_heartL = gizX2Y(set_heartL_RATIO,  set_heartL_ADDITION, issuedData->attrVals.valueset_heartL); 
    }
        
    if(0x01 == issuedData->attrFlags.flagset_spo2H)
    {
        info->event[info->num] = EVENT_set_spo2H;
        info->num++;
        dataPoints->valueset_spo2H = gizX2Y(set_spo2H_RATIO,  set_spo2H_ADDITION, issuedData->attrVals.valueset_spo2H); 
    }
        
    if(0x01 == issuedData->attrFlags.flagset_spo2L)
    {
        info->event[info->num] = EVENT_set_spo2L;
        info->num++;
        dataPoints->valueset_spo2L = gizX2Y(set_spo2L_RATIO,  set_spo2L_ADDITION, issuedData->attrVals.valueset_spo2L); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueset_heartH != cur->valueset_heartH)
    {
        GIZWITS_LOG("valueset_heartH Changed\n");
        ret = 1;
    }
    if(last->valueset_heartL != cur->valueset_heartL)
    {
        GIZWITS_LOG("valueset_heartL Changed\n");
        ret = 1;
    }
    if(last->valueset_spo2H != cur->valueset_spo2H)
    {
        GIZWITS_LOG("valueset_spo2H Changed\n");
        ret = 1;
    }
    if(last->valueset_spo2L != cur->valueset_spo2L)
    {
        GIZWITS_LOG("valueset_spo2L Changed\n");
        ret = 1;
    }
    if(last->valueheart_alarm != cur->valueheart_alarm)
    {
        GIZWITS_LOG("valueheart_alarm Changed\n");
        ret = 1;
    }
    if(last->valuespo2_alarm != cur->valuespo2_alarm)
    {
        GIZWITS_LOG("valuespo2_alarm Changed\n");
        ret = 1;
    }
    ...
    if(0 != memcmp((uint8_t *)&last->valueheart_hisdata,(uint8_t *)&cur->valueheart_hisdata,sizeof(last->valueheart_hisdata)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueheart_hisdata Changed\n");
            ret = 1;
        }
    }
    if(0 != memcmp((uint8_t *)&last->valuespo2_hisdata,(uint8_t *)&cur->valuespo2_hisdata,sizeof(last->valuespo2_hisdata)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuespo2_hisdata Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(heart_alarm_BYTEOFFSET,heart_alarm_BITOFFSET,heart_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valueheart_alarm);
    gizStandardCompressValue(spo2_alarm_BYTEOFFSET,spo2_alarm_BITOFFSET,spo2_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valuespo2_alarm);
    gizByteOrderExchange((uint8_t *)devStatusPtr->alertBitBuf,sizeof(devStatusPtr->alertBitBuf));

    devStatusPtr->valueset_heartH = gizY2X(set_heartH_RATIO,  set_heartH_ADDITION, dataPoints->valueset_heartH); 
    devStatusPtr->valueset_heartL = gizY2X(set_heartL_RATIO,  set_heartL_ADDITION, dataPoints->valueset_heartL); 
    devStatusPtr->valueset_spo2H = gizY2X(set_spo2H_RATIO,  set_spo2H_ADDITION, dataPoints->valueset_spo2H); 
    devStatusPtr->valueset_spo2L = gizY2X(set_spo2L_RATIO,  set_spo2L_ADDITION, dataPoints->valueset_spo2L); 
    ...
    gizMemcpy((uint8_t *)devStatusPtr->valueheart_hisdata,(uint8_t *)&dataPoints->valueheart_hisdata,sizeof(dataPoints->valueheart_hisdata));
    gizMemcpy((uint8_t *)devStatusPtr->valuespo2_hisdata,(uint8_t *)&dataPoints->valuespo2_hisdata,sizeof(dataPoints->valuespo2_hisdata));
    return 0;
}
